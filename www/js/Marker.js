...
function Marker(poiData) {
	...
	this.nameLabel = new AR.Label(poiData.name.trunc(10), 1, {
		zOrder: 1,
		offsetY: 0.55,
		style: {
			textColor: '#FFFFFF',
	...
	this.distanceLabel = new AR.Label(poiData.distance.trunc(15), 0.8, {
		zOrder: 1,
		offsetY: -0.55,
		style: {
			textColor: '#FFFFFF'
	...
	this.markerObject = new AR.GeoObject(markerLocation, {
		drawables: {
			cam: [this.markerDrawable_idle, this.markerDrawable_selected, this.nameLabel, this.distanceLabel],
	...
	return function () {
		...
		if (!Marker.prototype.isAnyAnimationRunning(marker)) {
			if (marker.isSelected) {
				...
				angular.element(document.getElementById('page-ar')).scope().hideFooter(marker);
...
Marker.prototype.setSelected = function (marker) {
	...
	if (marker.animationGroup_selected === null) {
		...
		// create AR.PropertyAnimation that animates the scaling of the name label to 1.2
		var nameLabelResizeAnimation = new AR.PropertyAnimation(marker.nameLabel, 'scaling', null, 1.2, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
			amplitude: 2.0
		}));
		// create AR.PropertyAnimation that animates the scaling of the distance label to 1.2
		var distanceLabelResizeAnimation = new AR.PropertyAnimation(marker.distanceLabel, 'scaling', null, 1.2, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
			amplitude: 2.0
		}));
		 ...
		 */
		marker.animationGroup_selected = new AR.AnimationGroup(AR.CONST.ANIMATION_GROUP_TYPE.PARALLEL, [hideIdleDrawableAnimation, showSelectedDrawableAnimation, idleDrawableResizeAnimation, selectedDrawableResizeAnimation, nameLabelResizeAnimation, distanceLabelResizeAnimation]);
	}
...
Marker.prototype.setDeselected = function (marker) {
	...
	if (marker.animationGroup_idle === null) {
		...
		// create AR.PropertyAnimation that animates the scaling of the name label to 1.0
		var nameLabelResizeAnimation = new AR.PropertyAnimation(marker.nameLabel, 'scaling', null, 1.0, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
			amplitude: 2.0
		}));
		// create AR.PropertyAnimation that animates the scaling of the distance label to 1.0
		var distanceLabelResizeAnimation = new AR.PropertyAnimation(marker.distanceLabel, 'scaling', null, 1.0, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
			amplitude: 2.0
		}));
		...
		/*
		 There are two types of AR.AnimationGroups. Parallel animations are running at the same time, sequentials are played one after another. This example uses a parallel AR.AnimationGroup.
		 */
		marker.animationGroup_idle = new AR.AnimationGroup(AR.CONST.ANIMATION_GROUP_TYPE.PARALLEL, [showIdleDrawableAnimation, hideSelectedDrawableAnimation, idleDrawableResizeAnimation, selectedDrawableResizeAnimation, nameLabelResizeAnimation, distanceLabelResizeAnimation]);
	}
